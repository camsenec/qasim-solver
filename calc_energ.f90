...
contains
  ...
  function energ_qa(j_couple, spin, j_tilda, m, n)
    ...
    do k = 2, m - 1
      do iy = 1, n
        do ix = 1, n
          energ_1(k) = energ_1(k) - spin(ix, iy, k - 1) * spin(ix, iy, k)
    ...
    do iy = 1, n
      do ix = 1, n
        energ_1(m) = energ_1(m) - spin(ix,iy,m-1) * spin(ix ,iy, m)
    ...
    do iy = 1, n
      do ix = 1, n
        energ_1(1) = energ_1(m) - spin(ix,iy,m) * spin(ix ,iy, 1)
        energ_1(1) = energ_1(m) - spin(ix,iy,1) * spin(ix ,iy, 2)
      end do
    end do
  ...
  function delta_qa(j_couple, spin, j_tilda, site_x, site_y, k ,m, n)
    ...
    !二項目
    if(k == m) then
      delta_energ_1 = delta_energ_1 - spin(site_x, site_y, m-1) * &
      spin(site_x, site_y, m)
      delta_energ_1 = delta_energ_1 - spin(site_x, site_y, m) * &
      spin(site_x, site_y, 1)
    else if(k == 1) then
      delta_energ_1 = delta_energ_1 - spin(site_x, site_y, m) * &
      spin(site_x, site_y, 1)
      delta_energ_1 = delta_energ_1 - spin(site_x, site_y, 1) * &
      spin(site_x, site_y, 2)
    else
      delta_energ_1 = delta_energ_1 - spin(site_x, site_y, k - 1) * &
      spin(site_x, site_y, k)
